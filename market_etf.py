...
# 레버리지·인버스·채권·해외지수는 '어느 테마냐'를 물어도 답이 없다.
# 테마 연결에서만 뺀다 (거래대금 순위에는 그대로 남는다).
NOT_THEME = re.compile(r"레버리지|인버스|２[Xx]|2[Xx]|채권|국고채|통안|머니마켓|"
                       r"금리|달러|엔화|원유|금선물|은선물|배당|리츠|TR|고배당")
